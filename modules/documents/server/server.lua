...
RegisterCallback('roots-search:server:getPlayerCopies', function(source, cb)
  local src = source
...
if Config.DocumentItemName then
  RegisterItem(Config.DocumentItemName, function(source)
    local src = source
    TriggerClientEvent("roots-search:client:useItem", src)
  end)
end

RegisterCallback('roots-search:server:getPlayerDocuments', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('roots-search:server:getDocumentTemplates', function(source, cb)
  local src = source  
...
RegisterCallback('roots-search:server:createTemplate', function(source, cb, data)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)
...
RegisterCallback('roots-search:server:editTemplate', function(source, cb, data)
...
RegisterCallback('roots-search:server:deleteTemplate', function(source, cb, data)
...
RegisterCallback('roots-search:server:createDocument', function(source, cb, data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterServerEvent('roots-search:server:createServerDocument')
AddEventHandler('roots-search:server:createServerDocument', function(data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('roots-search:server:deleteDocument', function(source, cb, data)
...
RegisterServerEvent('roots-search:server:giveCopy')
AddEventHandler('roots-search:server:giveCopy', function(data, targetId)
  local src = source
  local tsrc = targetId
  ...
  MySQL.Async.insert('INSERT INTO k5_documents (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
    ...
    TriggerClientEvent("roots-search:client:copyGave", src, data.name)
    TriggerClientEvent("roots-search:client:copyReceived", tsrc, data.name)
  end)

end)

RegisterServerEvent("roots-search:server:receiveDocument")
AddEventHandler("roots-search:server:receiveDocument", function(data, targetId)
  local tsrc = targetId

  MySQL.Async.fetchAll('SELECT data FROM k5_documents WHERE id = @docId', {
    ['@docId'] = data.docId,
  }, function(result)
    TriggerClientEvent("roots-search:client:viewDocument", tsrc, result[1])
  end)
end)
